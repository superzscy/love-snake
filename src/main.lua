...
local function isOpposite(a, b)
    return (a == "left" and b == "right") or (a == "right" and b == "left") or (a == "up" and b == "down") or
               (a == "down" and b == "up")
end
...
local function randomColor()
    return {0.35 + love.math.random() * 0.55, 0.35 + love.math.random() * 0.55, 0.35 + love.math.random() * 0.55}
end
...
function love.update(dt)
    if gameOver then
        ...
        local textHeight = gameOverFont:getHeight()
        love.graphics.setColor(0.95, 0.4, 0.35)
        love.graphics.print(text, math.floor((w - textWidth) / 2), math.floor((h - textHeight) / 2))
...
function love.draw()
    ...
    love.graphics.rectangle("fill", 0, fieldOffsetY, gridWidth * gridSize, gridHeight * gridSize)

    love.graphics.setColor(wallColor)
    love.graphics.setLineWidth(4)
    love.graphics.rectangle("line", 2, fieldOffsetY + 2, gridWidth * gridSize - 4, gridHeight * gridSize - 4)
    love.graphics.setLineWidth(1)

    love.graphics.setColor(0.1, 0.12, 0.14)
    for x = 1, gridWidth do
        love.graphics.line(x * gridSize, fieldOffsetY, x * gridSize, fieldOffsetY + gridHeight * gridSize)
    end
    for y = 1, gridHeight do
        love.graphics.line(0, fieldOffsetY + y * gridSize, gridWidth * gridSize, fieldOffsetY + y * gridSize)
    end

    for i = 1, #foods do
        local f = foods[i]
        love.graphics.setColor(f.color)
        love.graphics.circle("fill", (f.x - 0.5) * gridSize, fieldOffsetY + (f.y - 0.5) * gridSize, foodRadius)
    end
    ...
    for i = 1, #snake do
        ...
        love.graphics.rectangle("fill", (segment.x - 1) * gridSize + 2, fieldOffsetY + (segment.y - 1) * gridSize + 2,
            gridSize - 4, gridSize - 4, 6, 6)
    end
...
function drawHUD()
    ...
    local hudTextY = math.floor((hudHeight - 18) / 2)

    love.graphics.setColor(0.85, 0.88, 0.92)
    love.graphics.print(string.format("Time: %ds  Food: %d  %s", elapsed, foodCount, speedText), hudPadding, hudTextY)

    love.graphics.setColor(0.8, 0.85, 0.9)
    love.graphics.print("WASD / Arrow keys to move. R to reset.", 300, hudTextY)
    if gameOver then
        local text = "Game Over"
        local w, h = love.graphics.getDimensions()
        love.graphics.setFont(gameOverFont)
